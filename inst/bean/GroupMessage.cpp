...
namespace bean
{
	GroupMessage::GroupMessage(MessageBase &data) : m_data(data)
	{
	}

	QVariantMap GroupMessage::toJson()
	{
		QVariantMap ret;

		ret[bean::kszMsgType]         = bean::kszGroup;
	...
	QVariantMap GroupMessage::toMessageDBMap()
	{
		/*
	...
	QString GroupMessage::toMessageXml()
	{
		QDomDocument doc;
		...
		// message type
		root.setAttribute(bean::kszType, bean::kszGroupChat);

		// from & to
		root.setAttribute(bean::kszGroup, m_data.to);
