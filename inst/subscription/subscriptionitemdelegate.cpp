...
SubscriptionItemDelegate::SubscriptionItemDelegate(SubscriptionModel *model, QObject *parent)
...
QSize SubscriptionItemDelegate::sizeHint(const QStyleOptionViewItem & /*option*/, const QModelIndex & /*index*/) const
{
	return QSize(kCellWidth, kCellHeight);
}

void SubscriptionItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
	...
	QPixmap logo = m_model->subscriptionLogo(subscription.id());
	QString name = subscription.name();
