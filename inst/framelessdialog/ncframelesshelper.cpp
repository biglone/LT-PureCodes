...
void NcCursorPosCalculator::recalculate( const QPoint& globalMousePos, const QRect& frameRect )
{
  ...
  int frameWidth = fRect.width();
  int frameHeight = fRect.height();
  ...
  // qWarning() << "recalculate frame: " << fRect;
  // qWarning() << "global pos: " << globalMousePos;
  // qWarning() << "top:" << onTopEdge << " bottom:" << onBottomEdge << " left:" << onLeftEdge << " right:" << onRightEdge;
}
...
NcWidgetData::NcWidgetData( NcFramelessHelperImpl* _d, QWidget* topLevelWidget )
{
  ...
  mWidget->setWindowFlags( (mWindowFlags&Qt::WindowType_Mask)|Qt::CustomizeWindowHint|Qt::FramelessWindowHint );
...
void NcWidgetData::moveWidget( const QPoint& globalMousePos )
{
  ...
  else
  {
	...
	// qDebug() <<globalMousePos - mDragPos << mWidget->pos();
  }
}
...
void NcWidgetData::handleMousePressEvent( QMouseEvent* event )
{
  if ( event->button() == Qt::LeftButton )
  {
    ...
    mDragPos = mWidget->mapFromGlobal(event->globalPos());
...
{
	if ( mLeftButtonPressed && (event->buttons() & Qt::LeftButton))
  {
  ...
  {
    // qWarning() << "begin handleMouseMoveEvent"; 
    updateCursorShape( event->globalPos() );
	// qWarning() << "end handleMouseMoveEvent";
  }
...
void NcWidgetData::handleHoverMoveEvent( QHoverEvent* event )
{
  ...
  {
    // qWarning() << "begin handleHoverMoveEvent";
	// qWarning() << "frameGeometry: " << mWidget->frameGeometry();
	// qWarning() << "geometry: " << mWidget->geometry();
    updateCursorShape( mWidget->mapToGlobal( event->pos() ) );
	// qWarning() << "end handleHoverMoveEvent";
  }
}
