...
GlobalNotificationLastMsgItemDelegate::GlobalNotificationLastMsgItemDelegate(GlobalNotificationLastMsgModel *model, QObject *parent)
	: QItemDelegate(parent), m_model(model)
{
	Q_ASSERT(m_model);
}

GlobalNotificationLastMsgItemDelegate::~GlobalNotificationLastMsgItemDelegate()
{

}

QSize GlobalNotificationLastMsgItemDelegate::sizeHint(const QStyleOptionViewItem & /*option*/, const QModelIndex & /*index*/) const
{
	return QSize(68, 68);
}

void GlobalNotificationLastMsgItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
	...
	QPixmap logo = qPmApp->getModelManager()->globalNotificationLogo(id);
	...
	int unreadMsgCount = qPmApp->getGlobalNotificationMsgManager()->unreadMsgCount(id);
	if (unreadMsgCount > 0)
	{
		// unread count
