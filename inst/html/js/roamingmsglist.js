...
$(document).ready(function () {
	...
	try {
		...
		Message4Js.displayError.connect(displayError);
		...
		Message4Js.jsdebug("roaming msg load exception"  + e);
	}		
});
			...
			if (direct == "send") {
				...
				if (width > 0 && height > 0) {
					...
					str +="<image class='img_autodisplay' title='"+LANG_DICT["DOUBLE_CLICK_VIEW_IMAGE"]+"' origsrc=\""+value.path
					     +"\" src=\""+Message4Js.getAutodisplayImgSrc(value.path)+"\" width=\""+width+"\" height=\""+height+"\" secret=\""
...
function displayError() {
	RemoveAll();
	var emptyContainer = $("<div/>").addClass('empty').css('text-align', 'center').css('padding-top', '36px');
	emptyContainer.append("<img src='qrc:/html/images/errmsg.png' />");
	$(".chatBox_msgList").append(emptyContainer);
}
