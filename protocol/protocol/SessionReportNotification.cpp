...
namespace protocol
{
	int SessionReportNotifiction::In::getNotificationType()
	{
		return protocol::SESSION_REPORT;
	}

	bool SessionReportNotifiction::In::Parse( iks* pnIks )
	{
		bool isOk = false;
		do
		{
	...
	std::string SessionReportNotifiction::Out::getBuffer()
	{
		string sRet = "";
		...
		do
		{
			...
			iks* pnReport = iks_insert(pnMessage, protocol::TAG_REPORT);
			if (!pnReport) break;

			iks_insert_attrib(pnReport, protocol::ATTRIBUTE_NAME_ID, param.id.c_str());
			iks_insert_attrib(pnReport, protocol::ATTRIBUTE_NAME_FROM, param.from.c_str());
			iks_insert_attrib(pnReport, protocol::ATTRIBUTE_NAME_TO, param.to.c_str());
