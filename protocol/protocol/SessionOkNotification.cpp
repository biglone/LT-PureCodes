...
namespace protocol
{
    int SessionOkNotification::In::getNotificationType()
    {
        return protocol::SESSION_OK;
    }

    bool SessionOkNotification::In::Parse( iks* pnIks )
    {
        bool isOk = false;
        do
        {
    ...
    std::string SessionOkNotification::Out::getBuffer()
    {
        string sRet = "";
        ...
        do
        {
            ...
            iks* pnOk = iks_insert(pnMessage, protocol::TAG_OK);
            if (!pnOk) break;

            iks_insert_attrib(pnOk, protocol::ATTRIBUTE_NAME_ID, param.id.c_str());
            iks_insert_attrib(pnOk, protocol::ATTRIBUTE_NAME_FROM, param.from.c_str());
            iks_insert_attrib(pnOk, protocol::ATTRIBUTE_NAME_TO, param.to.c_str());

			// video
            if (param.hasVideo)
            {
                // video
                iks* pnVideo = iks_insert(pnOk, protocol::TAG_VIDEO);
                if (!pnVideo)
                    break;
            ...
            // audio
            iks* pnAudio = iks_insert(pnOk, protocol::TAG_AUDIO);
            if (!pnAudio)
                break;
