...
namespace protocol
{
	int SessionRejectNotification::In::getNotificationType()
	{
		return protocol::SESSION_REJECT;
	}

	bool SessionRejectNotification::In::Parse( iks* pnIks )
	{
		bool isOk = false;
		do
		{
	...
	std::string SessionRejectNotification::Out::getBuffer()
	{
		string sRet = "";
		...
		do
		{
			...
			iks* pnReject = iks_insert(pnMessage, protocol::TAG_REJECT);
			if (!pnReject) break;

			iks_insert_attrib(pnReject, protocol::ATTRIBUTE_NAME_ID, param.id.c_str());
			iks_insert_attrib(pnReject, protocol::ATTRIBUTE_NAME_FROM, param.from.c_str());
			iks_insert_attrib(pnReject, protocol::ATTRIBUTE_NAME_TO, param.to.c_str());

			iks *pnReason = iks_insert(pnReject, protocol::TAG_REASON);
			if (!pnReason) break;
